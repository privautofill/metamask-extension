...
class Driver {
  ...
  /**
   * Switches to a specific window tab using its ID and gets its title.
   *
   ...
   * @throws {Error} Throws an error if the window title does not load.
   */
  async getWindowTitleByHandleId(handleId) {
    await this.driver.switchTo().window(handleId);
    return await this.windowHandles.getCurrentWindowProperties(
      'title',
      handleId,
    );
  }
